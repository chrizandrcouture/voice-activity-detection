...
class FileManager:
    ...
    def prepare_files(self, normalize=False):
        ...
        if 'raw' not in self.data:
            ...
            self.data.create_dataset(
                'raw', (self.get_track_count(),), dtype=dt)
        ...
        for i, file in enumerate(self.data['files']):
            ...
            # Store data.
            self.data['raw'][i] = np.array(
                track.get_array_of_samples(), dtype=np.int16)
    ...
    def collect_frames(self):
        ...
        for raw in self.data['raw']:
            frame_count += int((len(raw) + (FRAME_SIZE -
                               (len(raw) % FRAME_SIZE))) / FRAME_SIZE)
            print('Counting frames ({0} of {1})'.format(
                progress, self.get_track_count()), end='\r', flush=True)
            progress += 1
        ...
        for raw in self.data['raw']:
            ...
            raw = np.concatenate(
                (raw, np.zeros(FRAME_SIZE - (len(raw) % FRAME_SIZE))))
            ...
            progress += len(frames)
            print('Merging frames ({0} of {1})'.format(
                progress, frame_count), end='\r', flush=True)
    ...
    def label_frames(self):
        ...
        for pos in range(0, frame_count, batch_size):
            ...
            labels = [1 if vad.is_speech(
                f.tobytes(), sample_rate=SAMPLE_RATE) else 0 for f in frames]
            self.data['labels'][pos: pos + batch_size] = np.array(labels)

            progress += len(labels)
            print('Labelling frames ({0} of {1})'.format(
                progress, frame_count), end='\r', flush=True)
...
def add_noise(speech_frames, noise_frames, align_frames, noise_level_db):
    ...
    # Overlay noise.
    track = noise_track.overlay(
        speech_track, gain_during_overlay=noise_level_db)
